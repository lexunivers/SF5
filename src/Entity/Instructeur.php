...
class Instructeur
{
    ...
    public function addFacture(Vol $facture): self
    {
        if (!$this->facture->contains($facture)) {
            $this->facture[] = $facture;
            $facture->setInstructeur($this);
        }

        return $this;
    }

    public function removeFacture(Vol $facture): self
    {
        if ($this->facture->removeElement($facture)) {
            // set the owning side to null (unless already changed)
            if ($facture->getInstructeur() === $this) {
                $facture->setInstructeur(null);
            }
        }
